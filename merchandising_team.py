...
try:
    ...
    for designer in orderLines[(orderLines["orderDate"] > time_limit_epochs) & (orderLines["isCancelled"] == False)][
        "designer"]:
    ...
    for price in orderLines[(orderLines["orderDate"] > time_limit_epochs) & (orderLines["isCancelled"] == False)][
        "rentPaid"]:
        price = get_range_hundreds(price)
    ...
    filtered_catalogue = catalogue[(pd.isna(catalogue["latestScan"]) == True) & (catalogue["status"] == True) & (
            catalogue["updateddate"] > date_limit) & (catalogue["disabled"] == False)]
    ...
    current_date_new = current_date.strftime("%d-%m-%Y")
    lst1 = list()
    lst1.append(current_date_new)

    googleSheet.update_sheet(lst1, "AU")
    save_last_updated_date(current_date_new)

except:
    current_date_new = get_last_updated_date()
    googleSheet.update_sheet(lst1, "AU")
